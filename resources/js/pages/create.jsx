...
export default function Create() {
  ...
  const [headers, setHeaders] = useState(["Size", "Bust", "Waist", "Hip"]);
 ...
 const exportTable = () => {
  ...
  const [imageUrls, setImageUrls] = useState(presets['0'].url);
...
const handleSave = () => {
  ...
  <button onClick={handleDiscard}>Discard</button>
    ...
    <input type='hidden' value={shop}  name='shopname'/>
        <Grid>
              ...
              <thead>
                <tr>
                  {headers.map((header, idx) => (
                    <th key={idx} style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                      {header}
                    </th>
                  ))}
                </tr>
